		...
		for(int i = 0; i < collisionData.size(); i++)
		{
			for(int k = 0; k < temp.size(); k++)
			{
				if(Math.abs(temp.get(k).x - collisionData.get(i).x) < 1 && Math.abs(temp.get(k).y - collisionData.get(i).y) <1)
				{
					...
					switch(parent.type)
					{
					case 'R' : parent.levGen.getRandomLev(); break;
					
					
				default:	for (int j = 0; j < levGen.system.length; j++)
						{
						if(levGen.system[j] == parent)
						{
							switch(j)
							{
							case 0 : levGen.calculateLevel1(); break;
							case 1 : levGen.calculateLevel2(); break;
							case 2 : levGen.calculateLevel3(); break;
							case 3 : levGen.calculateLevel4(); break;
							case 4 : levGen.calculateLevel5(); break;
							case 5 : levGen.calculateLevel6(); break;
							}
